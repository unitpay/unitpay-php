...
The SDK adds a small, **anonymous** version fingerprint to the requests it
already makes, so Unitpay can see which SDK/PHP versions are in the field. This
...
That is the whole of it — there is no separate telemetry endpoint, no opt-in
beacon, and nothing to configure.
